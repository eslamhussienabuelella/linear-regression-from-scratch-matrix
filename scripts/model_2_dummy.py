...
from src.design_matrices import X_dummy
...
def main():
    ...
    print("Model 2 (Dummy Variable: Parallel Lines)")
    print("Beta (intercept, slope, dummy):\n", beta)
    print("R²:", r2)

    # Extended x-range for plotting fitted lines
    x_ext = np.arange(-1, 9)

    # Predict for group 1 (g=1) and group 0 (g=0)
    X_ext_g1 = X_dummy(x_ext, np.ones_like(x_ext))
    X_ext_g0 = X_dummy(x_ext, np.zeros_like(x_ext))
    ...
    plt.xlabel("x")
    plt.ylabel("y")
    plt.title("Model 2: Dummy Variable (Parallel Lines)")
    plt.grid()
    plt.legend()

    plt.savefig("figures/model2_fit_parallel_lines.png",
    ...
    plt.savefig("figures/model2_residuals_vs_fitted.png",
                dpi=300, bbox_inches="tight")
    plt.show()
    plt.close()
